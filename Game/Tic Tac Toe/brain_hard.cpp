...
namespace Liron486
{
	...
	static int RecCheckBestMove(Board& board_copy, char type, const Judge& judge, const Point& last_move, int depth)
	{
		...
		for (int i = 0; i < 9; ++i)
		{
			...
			{
				current_value = RecCheckBestMove(board_copy, type, judge, nextMove, depth + 1);
	...
	static bool CheckIfWinNextMove(Board& board, std::vector<Point>& best_moves, Judge& judge, char type, int range, int& index_result)
	{
	...
	Point BrainHard::GetNextMove() const
	{
		...
		else
		{
			for (int i = 0; i < 9; ++i)
			{
				...
				{
					current_res = RecCheckBestMove(board_copy, m_myType, judge, checkMove, 1);
					if (current_res > max)
					{
						best_moves.clear();
						max = current_res;
			...
			if (CheckIfWinNextMove(board_copy, best_moves, judge, m_myType, counter_best_moves, index_of_winning_move))
			{
