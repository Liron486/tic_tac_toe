...
namespace Liron486
{
	...
	void Gui::PrintHeader(const Score& score_, int gameNUmber_, Configuration::Difficulty difficulty_) const
	{
		std::string difficulty = difficulty_ == Configuration::Difficulty::EASY ? "EASY" : "HARD";

		std::cout << "      " << score_.GetPlayersNames()[0] << " - " << score_.GetWinsCounter()[0] << "  |  " <<
			score_.GetPlayersNames()[1] << " - " << score_.GetWinsCounter()[1] << "  ||||  Difficulty: " << difficulty << "  |  Game Number: " << gameNUmber_ << std::endl;
	...
	{
		std::cout << "      " << score_.GetPlayersNames()[0] << " - " << score_.GetWinsCounter()[0] << "  |  " <<
			score_.GetPlayersNames()[1] << " - " << score_.GetWinsCounter()[1] << "  ||||  Game Number: " << gameNUmber_ << std::endl;

		std::cout << "------------------------------------------------------------" << std::endl;
	}

	void Gui::WeHaveAWinner(Player** players_, int playerNum_) const
	{
		std::cout << "We have a Winner!!!!!\n" << players_[playerNum_]->GetName() << " Wins!\n" << std::endl;
	...
	void Gui::Tutorial(const std::vector<std::vector<char> >& board_, const Score& score_, Configuration::Difficulty difficulty_, const std::string& name1, const std::string& name2) const
	{
